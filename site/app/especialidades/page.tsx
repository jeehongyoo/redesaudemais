  ...
  return (
    <section className="section-rythm">
      <div className="container-site">
        ...
        <h1 data-words className="sec-title font-display text-3xl font-bold text-[#162B4D] md:text-4xl">Especialidades</h1>
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3" style={{ gridAutoRows: "1fr" }}>
          {SPECIALTIES.map((s, i) => (
            <div key={s.slug} data-reveal="mask" data-tilt data-reveal-i={String(i)} className={`${card}${i === 0 ? " bento-featured sm:col-span-2 lg:col-span-2" : ""}`}>
              ...
              <p className={`mt-5 font-display font-semibold text-[#162B4D] ${i === 0 ? "text-2xl md:text-[28px]" : "text-lg"}`}><span className="card-title-line">{s.name}</span></p>
