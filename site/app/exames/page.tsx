  ...
  return (
    <section className="section-rythm">
      <div className="container-site">
        ...
        <h1 data-words className="sec-title font-heading text-3xl font-semibold text-[#162B4D] md:text-4xl">Exames</h1>
        ...
        <div className="grid gap-5 md:grid-cols-2" style={{ gridAutoRows: "1fr" }}>
          {EXAMS.map((e, i) => (
            <div key={e.name} data-reveal="mask" data-tilt data-reveal-i={String(i)} className={`${card}${i === 0 ? " bento-featured sm:col-span-2 lg:col-span-2" : ""}`}>
              ...
              <p className={`mt-5 font-heading font-semibold text-[#162B4D] ${i === 0 ? "text-2xl md:text-[28px]" : "text-lg"}`}><span className="card-title-line">{e.name}</span></p>
